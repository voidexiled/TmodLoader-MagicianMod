...
public class CardChancesUI : UIState
{
    ...
    public override void Draw(SpriteBatch spriteBatch)
    {
        ...
        if (!player.ShouldShowMagicianUI()) return;

        var maxPileLength = player.MaxCardsPileLength;
        ...
        if (currentPile.Count > 0)
            for (var i = 0; i < currentPile.Count; i++)
            {
                ...
                var texturePosition =
                    new Vector2(
                        screenOffsetX + currentPile.Count * texture.Width() - i * texture.Width() - texture.Width(),
                        _offsetY);
